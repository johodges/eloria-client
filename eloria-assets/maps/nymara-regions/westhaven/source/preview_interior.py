...
INTERIOR_LIGHT = R.Lighting(
    ...
    sun_color=(0.20, 0.17, 0.30),
    sky_color=(0.07, 0.06, 0.10),
    ground_color=(0.04, 0.04, 0.05),
    fog_color=(0.048, 0.042, 0.062),
    fog_density=0.014,
    fog_height_falloff=0.0,
    exposure=1.12,
    ambient_strength=0.30,
    saturation=1.10,
    sky_zenith=(0.05, 0.05, 0.07),
    sky_horizon=(0.09, 0.08, 0.11),
)
...
def main() -> int:
    ...
    if args.captures:
        ...
        for i, (ident, subject, space, camera, repeat) in enumerate(views):
            ...
            # Slugified, not just space-substituted. Subjects contain a colon
            # ("The Custom House: the tide board"), and on Windows a colon in a
            # path is the NTFS alternate-data-stream separator: `image.save`
            # succeeds, the name is silently truncated at the colon, and every
            # capture lands on disk as a zero-byte file. The whole first
            # Westhaven interior capture run came out empty that way, and the
            # index still pointed at names that looked right.
            name = _slug(f"{ident}-{subject}")
