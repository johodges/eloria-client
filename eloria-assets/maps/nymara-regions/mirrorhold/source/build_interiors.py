#!/usr/bin/env python3
"""Build the Mirrorhold interior map packages.
...
ROOT = Path(__file__).resolve().parents[1]           # .../mirrorhold
...
def export_glb(interior: I.Interior, sets, path: Path):
    builder = GLTF.GltfBuilder(generator="Eloria Mirrorhold interior builder")
...
def write_manifest(interior: I.Interior, stats, collision_stats, path: Path):
    ...
    doc = {
        ...
        "portals": [{
            "id": "exit-to-amberwood",
            "name": "Return to Amberwood",
            "type": "map-transition",
            "position": spawn,
            "radius": 3.0,
        ...
        # Cool, because Mirrorhold's vessels are blue crystal. Amberwood's
        # interiors are lit by amber resin and read warm; copying that colour
        # here would light a glacier city like a timber hall.
        ...
        "conceptArt": {
            "detailBoard": "../../amberwood/references/00-concept-detail-board.png",
        ...
        "sources": [
            ...
            {"id": "anchor", "landmark": interior.anchor_landmark,
             ...
             "role": "surface entrance on the Amberwood region map"},
        ],
        "provenance": {
            "geometry": "authored with the Amberwood region toolkit; no scattered placement",
