...
def review(baseline,server):
    ...
    collision=with_step_mask(CollisionMap(384,384,final.tobytes()),2)
    reachable=flood(collision,[(116,116)])
    ...
    for p in w['interactives']:
        ...
        tile=p.get('serverTile') or to_tile(p['position'],[116,116],1)
    ...
    for name,pos in w['contentLayout']['npcs'].items():
        tile=to_tile(pos,[116,116],1);near=placer.nearest_fit(*tile,limit=3)
    ...
    return {'extentMetres':384,'origin':[116,116],'arrival':[116,116],
