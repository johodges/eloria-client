#!/usr/bin/env python3
"""Build the Grey Moors interior map package.
...
ROOT = Path(__file__).resolve().parents[1]           # .../grey_moors
...
def export_glb(interior: I.Interior, sets, path: Path):
    builder = GLTF.GltfBuilder(generator="Eloria Grey Moors interior builder")
...
def write_manifest(interior: I.Interior, stats, collision_stats, path: Path):
    ...
    doc = {
        ...
        "conceptArt": {
            "detailBoard": "../grey_moor_barrows/references/00-concept-detail-board.png",
...
def main() -> int:
    ...
    for key, builder_fn in (("grey_moors_insides", I.combine),):
        if args.only and key not in args.only:
            continue
        t0 = time.time()
        interior = builder_fn(SEED)
        # a secret whose door is down here rather than on the region above
        SD.dress_interior(interior, SEC, "grey_moor_barrows", SEED)
