"""One-time migration of Grey Moors posts, including portals returning from other maps.
...
def migrate(server):
    ...
    if data.get('grey_moors_landscape_revision')=='inhabited-384-v1':return
    ...
    for filename,rules in spec.items():
        ...
        for line in p.read_text().splitlines():
            ...
            for keyword,mi,xi,yi in rules:
                ...
                if fields[mi].strip()!='grey_moors':continue
    ...
    for key in ('grey_moors_npcs','grey_moors_harvestables'):
        if key in data:data[key]=follow(data[key])
    for entry in data.get('maps',[]):
        if entry.get('id')=='grey_moors':
            entry.update(follow(entry));entry['server_cells']=384;entry['arrival']=PLAN.tile((174,174))
    data['grey_moors_landscape_revision']='inhabited-384-v1'
