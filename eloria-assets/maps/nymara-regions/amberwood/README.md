...
The shared toolkit these call lives in `../_toolkit/`; only the build scripts
and this region's own composition remain here.
...
Note the cache: `preview.py` keys its texture cache on a digest of
`textures.py` and `materials.py`, so editing a recipe invalidates it. Before
that keying existed, a stale cache silently shipped textures that no longer
matched the source, and it is why this package once drifted from the code that
produces it. See `change-log.md`.
