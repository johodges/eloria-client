...
def main() -> int:
    ...
    if args.captures:
        ...
        for i, (ident, subject, space, camera, repeat) in enumerate(views):
            ...
            # Windows will not take ':' in a filename, and a combined insides
            # map's subject is "<Section name>: <subject>". Godot's save_png
            # silently produced four zero-byte files named up to the colon
            # before this was sanitised, and the frames after it were lost.
            safe = "".join(ch if (ch.isalnum() or ch in "-_") else "-"
                           for ch in subject.replace(" ", "-"))
            while "--" in safe:
                safe = safe.replace("--", "-")
