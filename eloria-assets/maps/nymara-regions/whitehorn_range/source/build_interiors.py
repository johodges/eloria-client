#!/usr/bin/env python3
"""Build the Whitehorn Range interior map packages.
...
`export_glb` and `build_collision` below are lifted unchanged from
`amberwood/source/build_interiors.py`. They carry no region-specific data and
belong in `_toolkit/`. They are duplicated here rather than promoted because
Amberwood's package was regenerated only recently and must stay byte-
reproducible; moving code out from under its build without re-verifying it is
how a reproducibility claim quietly stops being true. Promoting them and
re-verifying both regions in one commit is the right follow-up.

    python3 build_interiors.py
    python3 ../../_toolkit/verify_runtime.py \
        --package ../../interiors/whitehorn_glacier_temple
"""
...
def export_glb(interior: I.Interior, sets, path: Path):
    builder = GLTF.GltfBuilder(generator="Eloria Whitehorn interior builder")
...
def write_manifest(interior: I.Interior, stats, collision_stats, path: Path):
    ...
    doc = {
        ...
        "navigation": {
            ...
            "notes": ["Every standable surface is a Walk_<material> node: the "
                      "client turns navigation.surfaceNodePrefixes into the "
                      "layer its downward grounding ray tests, so a floor "
        ...
        "lights": [{"id": "lantern-%02d" % i, "kind": "point", "position": p,
                    "colour": [1.0, 0.72, 0.42], "range": 9.0, "energy": 1.5}
                   for i, p in enumerate(interior.lamps)],
        "environment": dict(interior.environment,
        ...
        "conceptArt": {
            ...
            "role": ("the interior's own board; TRUNCATED to 786,446 bytes in "
                     "the supplied package and does not decode, so the ten "
                     "subjects below come from concept.json and the parent "
                     "region's intact board, not from panel images"),
            "panelGrid": [5, 2],
            "viewCount": 10,
        ...
        "sources": [
            ...
            {"id": "plan", "file": "source/interiors_temple.py",
             "role": "region-owned interior composition"},
            {"id": "anchor", "landmark": interior.anchor_landmark,
             "position": interior.anchor_position,
             "role": "surface entrance on the Whitehorn Range region map"},
        ],
        "provenance": {
            "geometry": ("authored with the shared Nymara toolkit and this "
                         "region's own kit.py; no scattered placement"),
...
def main() -> int:
    ...
    for key, builder_fn in (("whitehorn_insides", IT.combine),):
        ...
        SD.dress_interior(interior, SEC, "whitehorn_glacier_temple", SEED)
        ...
        write_manifest(interior, stats, collision_stats, out / "world.json")
        ...
        counts = payload_json["issues"]
        ok = counts["numErrors"] == 0
        summary.append((interior.ident, stats["uniqueTriangles"],
                        stats["glbBytes"], collision_stats["walkableCells"], ok))
        print("[%s] %d tris, %.2f MB, %d walkable cells, "
              "glTF errors=%d warnings=%d (%.1fs)"
              % (interior.ident, stats["uniqueTriangles"],
                 stats["glbBytes"] / 1e6, collision_stats["walkableCells"],
                 counts["numErrors"], counts["numWarnings"],
                 time.time() - t0))
    ...
    for ident, tris, size, cells, ok in summary:
        print("  %-30s %7d tris %7.2f MB %6d cells %s"
              % (ident, tris, size / 1e6, cells, "PASS" if ok else "FAIL"))
