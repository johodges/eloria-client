"""One-time migration of Whitehorn posts, including portals returning from other maps.
...
from landscape_plan import PLAN
...
def migrate(server):
    ...
    if data.get('whitehorn_range_landscape_revision')=='inhabited-396-v1':return
    ...
    for filename,rules in spec.items():
        ...
        for line in p.read_text().splitlines():
            ...
            for keyword,mi,xi,yi in rules:
                ...
                if fields[mi].strip()!='whitehorn_range':continue
    ...
    for key in ('whitehorn_range_npcs','whitehorn_range_harvestables'):
        if key in data:data[key]=follow(data[key])
    for entry in data.get('maps',[]):
        if entry.get('id')=='whitehorn_range':
            entry.update(follow(entry));entry['server_cells']=396;entry['arrival']=PLAN.tile((162,105))
    data['whitehorn_range_landscape_revision']='inhabited-396-v1'
