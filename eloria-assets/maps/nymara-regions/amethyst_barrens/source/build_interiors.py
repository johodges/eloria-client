#!/usr/bin/env python3
"""Build the Amethyst Barrens interior map packages.
...
ROOT = Path(__file__).resolve().parents[1]           # .../amethyst_barrens
...
def export_glb(interior: I.Interior, sets, path: Path):
    builder = GLTF.GltfBuilder(generator="Eloria Amethyst Barrens interior builder")
...
def main() -> int:
    ...
    for key, builder_fn in (("amethyst_barrens_insides", I.combine),):
        ...
        SD.dress_interior(interior, SEC, "resonant_vault", SEED)
