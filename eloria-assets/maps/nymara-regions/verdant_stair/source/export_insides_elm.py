#!/usr/bin/env python3
"""Write the server-side ELM for the Verdant Stair insides map.
...
def main() -> int:
    ...
    ap.add_argument("--package",
                    default=str(here.parents[1] / "interiors" / "verdant_stair_insides"))
    ap.add_argument("--out",
                    default=str(here.parents[1] / "source-elm" / "verdant_stair_insides.elm"))
    # Only the magic is read off the template; the header is rebuilt from
    # scratch below. The region's own ELM is used rather than the insides map's,
    # which does not exist until this script has run once.
    ap.add_argument("--template",
                    default=str(here.parents[1] / "source-elm" / "verdant_stair.elm"))
