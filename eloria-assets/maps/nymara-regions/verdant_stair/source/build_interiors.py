#!/usr/bin/env python3
"""Build the Verdant Stair interior map packages.
...
ROOT = Path(__file__).resolve().parents[1]           # .../verdant_stair
...
def _ensure_walk_faces_up(name: str, piece: M.Mesh) -> M.Mesh:
    """Flip a walk surface with no upward-facing triangle.

    Godot's navigation collision is one-sided for raycasts, so a floor wound the
    wrong way is invisible to the grounding ray and nothing else in the pipeline
    notices. See the same guard in the region build for what found it.
    """
    ...
    good = lengths > 1e-9
    if not good.any() or ((normals[good, 1] / lengths[good]) > 0.35).any():
        return piece
    piece.flip_winding()
...
def export_glb(interior: I.Interior, sets, path: Path):
    builder = GLTF.GltfBuilder(generator="Eloria Verdant Stair interior builder")
    ...
    def emit(bucket: dict[str, M.Mesh], prefix: str, parent: GLTF.Node,
        ...
        for material, piece in sorted(bucket.items()):
            ...
            if walk:
                piece = _ensure_walk_faces_up(name, piece)
    ...
    emit(interior.group.by_material(walk=True), "Walk", root, walk=True)
...
def main() -> int:
    ...
    for key, builder_fn in (("verdant_stair_insides", I.combine),):
        if args.only and key not in args.only:
            continue
        t0 = time.time()
        interior = builder_fn(SEED)
        # a secret whose door is down here rather than on the region above
        SD.dress_interior(interior, SEC, "verdant_stair_insides", SEED)
