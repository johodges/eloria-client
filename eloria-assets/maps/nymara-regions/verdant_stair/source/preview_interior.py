...
# Amberwood's interiors are lit by hung amber and Amethyst's by crystal. These
# are lit by hung amber too, but the surfaces they fall on are jade, wet
# limestone and bark rather than slate and brass, so the ambient floor carries a
# green cast: a warm key alone on green stone reads as a sepia photograph.
INTERIOR_LIGHT = R.Lighting(
    sun_direction=(-0.28, 0.82, 0.50),
    sun_color=(0.34, 0.30, 0.21),
    sky_color=(0.06, 0.09, 0.08),
    ground_color=(0.04, 0.05, 0.04),
    fog_color=(0.044, 0.055, 0.050),
    fog_density=0.013,
    fog_height_falloff=0.0,
    exposure=1.52,
    ambient_strength=0.46,
    saturation=1.12,
    sky_zenith=(0.04, 0.06, 0.05),
    sky_horizon=(0.07, 0.10, 0.09),
)
...
def main() -> int:
    ...
    if args.captures:
        ...
        for i, (ident, subject, space, camera, repeat) in enumerate(views):
            ...
            # The subject is prose and reaches the filesystem as a file name.
            # A colon in it - "The Green Sanctum: the seated figure" - is a
            # drive separator on Windows, and the Godot capture pass silently
            # wrote directories instead of PNGs because of it. Keep only
            # characters that are legal in a file name everywhere.
            safe = "".join(c if (c.isalnum() or c in "-_") else "-"
                           for c in subject.replace(" ", "-"))
            name = f"{ident}-{safe}".strip("-")
            while "--" in name:
                name = name.replace("--", "-")
