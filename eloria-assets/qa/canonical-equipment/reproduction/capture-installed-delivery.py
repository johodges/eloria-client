...
for race in ['luminous_male','luminous_female','ssarathi_female','mycelari_female']:
    for clip,view in [('Idle_Subtle','front'),('Run_Female','side')]:
        subprocess.run([shutil.which('pwsh'),'-NoProfile','-File','eloria-assets/tools/capture_canonical_equipment.ps1','-Slugs',race,'-Prefix','installed-militia','-Outfit','0:114,3:133,4:219,5:208,6:248','-Clips',clip,'-Angles',view],check=True)
        a=PREVIEW/f'final-militia_{race}_{clip}_{view}.png';b=PREVIEW/f'installed-militia_{race}_{clip}_{view}.png'
...
(BUILD/'reports/installed-image-comparison.json').write_text(json.dumps(checks,indent=2)+'\n')
