"""Build original headwear with an immutable eye-based brow proposal."""
...
PROPOSAL=ROOT/'equipment-fit-build/in/eye-brow-proposal.py'
...
ORIGINAL['torso_remap.py']=r.digest(ROOT/'equipment-fit-build/in/outer-sleeve-proposal.py')
CODES=dict(ORIGINAL,**{'limb_head_remap.py':SOURCE_SHA})
DRIVER_SHA=r.digest(__file__)
module=types.ModuleType('head_candidate');module.__file__=str(ROOT/'eloria-assets/tools/limb_head_remap.py')
...
TAG='canonical-eyes';ROSTER={p.slug:p for p in r.batch.roster() if p.part==3}

def check_tools():
 actual=r.tool_hashes()
 assert all(actual[k]==v for k,v in ORIGINAL.items() if k!='limb_head_remap.py')
 assert actual['limb_head_remap.py'] in {ORIGINAL['limb_head_remap.py'],SOURCE_SHA}
...
def build(args):
 ...
 rig=r.cached_rig(race);report=module.build(piece.source,dest,rig,piece.kind,piece.name)
 check_tools();assert r.digest(source)==before and r.digest(body)==body_sha
 report.update(source_sha256=before,body_sha256=body_sha,tool_sha256=CODES,race=race,slug=slug,key=f'3:{piece.visual}',asset_sha256=r.digest(dest),orchestrator_sha256=DRIVER_SHA)
 report['variant']={'scene':dest.as_posix(),'authoredFor':race,'fitProfile':'canonical','socket':{'bone':'Head','offset':(r.ce.socket_origin(rig,3)-rig.origin('Head')).tolist(),'rotationDegrees':[0,0,0]}}
...
if __name__=='__main__':
 ...
 base=ROOT/'equipment-fit-build/out/canonical-sleeves/equipment.json'
 ...
 print('HEAD_CANDIDATES_COMPLETE',len(reports),flush=True)
