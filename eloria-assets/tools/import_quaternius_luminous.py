...
def import_animations(path: Path, output: Path):
    ...
    result={"schema":1,"source":"Quaternius Universal Animation Library","clips":{}}
    for target, source in CLIPS.items():
        ...
        for channel in animation["channels"]:
            ...
            bone=TARGET_BONES.get(node_names[channel["target"]["node"]])
            if bone is None: continue
            sampler=animation["samplers"][channel["sampler"]]
            times=accessor(document,binary,sampler["input"]); values=accessor(document,binary,sampler["output"])
            duration=max(duration,float(times[-1]))
            tracks[str(bone)]=[[round(float(time),6),runtime_quaternion(value)]
