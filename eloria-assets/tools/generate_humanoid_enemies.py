...
def animation(path,duration,poses):
    ...
    for bone in tracks:
        track=ET.SubElement(root,"TRACK",BONEID=str(bone),NUMKEYFRAMES=str(len(poses)),TRANSLATIONREQUIRED="0",TRANSLATIONISDYNAMIC="0",HIGHRANGEREQUIRED="0")
...
def append_actor_defs(path):
    ...
    for index,(slug,label,family,*_) in enumerate(ENEMIES):
        ...
        for tag,name in frames.items(): ET.SubElement(frame_root,tag).text=f"animations/enemies/{name}"
