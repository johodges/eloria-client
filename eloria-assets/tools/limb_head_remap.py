...
def leg_frame(points, faces, rig, full_harness=False):
    ...
    height = high - low
    floor = (-0.025 if full_harness else 0.10) * rig.fit_scale
    waist = 1.075 * rig.fit_scale
...
def boot_frame(points, faces, rig):
    ...
    for side, sign in [("l", 1.0), ("r", -1.0)]:
        ...
        target[1] = rig.positions[:, 1].min() - 0.004 * rig.fit_scale
...
def backing(
    rig, armour, faces, region, low, high, join_default_clothing=True, join_lower=True
):
    """A closed stocking/trouser lining inside the artwork, with body weights."""
    p, f = rig.positions, rig.faces
    c = p[f].mean(axis=1)
    ...
    _, first, weld = np.unique(
        np.round(p, 6), axis=0, return_index=True, return_inverse=True
    )
    ...
    for i in range(len(p)):
        side = "l" if p[i, 0] >= 0 else "r"
        if region == "boots" or p[i, 1] < 0.79 * rig.fit_scale:
            rows, sections = profiles[side]
            if len(rows):
                cx, cz, rx, rz = [
                    np.interp(p[i, 1], rows, sections[:, j]) for j in range(4)
                ]
        ...
        if np.isfinite(hit) and hit > 0.015 * rig.fit_scale:
            factor = min(
                factor, max(0.025 * rig.fit_scale, hit - 0.008 * rig.fit_scale) / length
            )
    ...
    walls = [(b, a, a + count) for a, b in boundary] + [
        (b, a + count, b + count) for a, b in boundary
    ]
    triangles = np.vstack(
        (f, f[:, ::-1] + count, np.asarray(walls, dtype=int).reshape(-1, 3))
    )
    return (
        np.vstack((outer, inner)),
        np.vstack((n, -n)),
        np.vstack((p[:, [0, 1]], p[:, [0, 1]])),
        triangles.ravel(),
...
def build(source, out, rig, kind, label, span=None):
    ...
    if full:
        ...
        p, surface.normals, surface.uvs, f = clipped
    ...
    if is_head:
        ...
        lower = (
            ...
            if region == "legs"
            else rig.positions[:, 1].min() - 0.005 * rig.fit_scale
        )
        upper = (
            1.065 * rig.fit_scale
            if region == "legs"
