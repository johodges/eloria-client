...
| Knowledge/books | legacy `knowledge.c`, `knowledge.h`, `multiplayer.c`; server `knowledge.py`, `protocol.py`, `server.py`, `world.py`, `config/items.txt`, `config/books.txt` | hashed data-driven catalog, ownership reducer, inspection window | exact bitset/new/index/text fixtures and UI bounds; local TCP synchronization and inspection pending | IMPLEMENTED |
