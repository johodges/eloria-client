#!/usr/bin/env python3
"""Deterministically migrate certified Mirrorhold into authoring.

Mirrorhold's current composed output is the bootstrap authority. The adapter
captures exact final object subtrees, its published crossing/access assemblies,
resolved terrain and water authority. Normal builds consume only the saved
scene and copied assets; they never reopen ``composed.pkl``.
"""
...
REGION_ID = "mirrorhold"
IMMUTABLE_PROFILE_PREFIX = (
    "eloria-assets/maps/nymara-regions/_continent/legacy-server-profile/"
)
TRANSLATION = np.array([840.0, 0.0, 650.0])
GRID_ORIGIN = np.array([-208.0, -190.0])
GRID_SHAPE = (271, 271)  # X vertices, Z vertices.
GRID_BYTES = GRID_SHAPE[0] * GRID_SHAPE[1] * 4
COLOR_BYTES = GRID_BYTES
PREVIEW_UV_DENSITY = 0.17
OWNED_ROUTE_IDS = tuple(sorted((
    "amberwood--mirrorhold-mirrorhold",
    "amethyst_barrens--mirrorhold-mirrorhold",
    "four_gates--mirrorhold-mirrorhold",
    "mirrorhold--sunmane_steppe-mirrorhold",
    "mirrorhold--verdant_stair-mirrorhold",
    "door-mirrorhold-lens-vault-stair",
    "door-mirrorhold-cistern-door",
    "door-mirrorhold-stair-cellars-door",
    "street-mirrorhold-west-bench-yard-link",
    "street-mirrorhold-lower-terrace-yard-link",
    "street-mirrorhold-mid-bench-yard-link",
    "street-mirrorhold-east-bench-yard-link",
    "street-mirrorhold-east-stair-home-yard-link",
    "street-mirrorhold-lake-north-yard-link",
    "street-mirrorhold-west-shore-yard-link",
    "street-mirrorhold-south-shore-yard-link",
    "street-mirrorhold-civic-bridge-exit",
    "street-mirrorhold-watch-hatch-approach",
    "street-mirrorhold-arrival-lane",
    "street-mirrorhold-civic-ascent",
    "street-mirrorhold-citadel-service-road",
    "street-mirrorhold-lens-vault-lane",
    "street-mirrorhold-orrery-pilgrim-road",
    "street-mirrorhold-quay-descent",
    "street-mirrorhold-east-working-road",
    "street-mirrorhold-quarry-road",
    "street-mirrorhold-lens-yard-lane",
    "street-mirrorhold-cistern-yard-lane",
    "street-mirrorhold-rose-gallery-lane",
    "street-mirrorhold-western-lens-lane",
    "street-mirrorhold-east-stair-yard",
    "street-mirrorhold-overlook-lane",
    "street-mirrorhold-south-watch-lane",
    "street-mirrorhold-reed-cut-lane",
    "street-mirrorhold-quarry-adit-lane",
    "street-mirrorhold-icebore-lane",
    "street-mirrorhold-canal-sluice-lane",
    "street-mirrorhold-sanctuary-causeway-approach",
    "street-mirrorhold-stair-town-0",
    "street-mirrorhold-stair-town-1",
    "street-mirrorhold-stair-town-2",
    "discovery-mirrorhold-73",
    "discovery-mirrorhold-606",
    "discovery-mirrorhold-607",
    "discovery-mirrorhold-608",
    "discovery-mirrorhold-612",
    "trail-mirrorhold-0",
    "access-mirrorhold-north-plateau",
)))
REQUIRED_ROUTE_IDS = tuple(identity for identity in OWNED_ROUTE_IDS
                           if "--" in identity or identity.startswith("door-"))
RIVER_ID = "mirror_outlet"
GAMEPLAY = (
...
PUBLISHED_ASSEMBLIES = {
    "mirror_outlet@16": (
        "Walk_ContinentalBridgeUnion_008_mirrorhold",
        "BridgeUnionTimberEdge_008_mirrorhold"),
    "mirror-bank-ramp-quay": (
        "Walk_Mirror_BankRamp_Quay", "Mirror_BankRamp_Foundation_Quay"),
    "mirror-bank-ramp-sanctuary": (
        "Walk_Mirror_BankRamp_Sanctuary",
        "Mirror_BankRamp_Foundation_Sanctuary"),
}
...
def final_object_records(content, objects: tuple[dict, ...]):
    ...
    if len(records) != 168 or len({entry["id"] for entry in records}) != 168:
        raise ValueError(
            f"Certified final Mirrorhold composition must contain 168 unique objects, got {len(records)}")
...
def terrain_colors(world, published_document: dict, published_body: bytes,
    ...
    try:
        ...
        raise ValueError(f"Mirrorhold color grid is outside certified composed terrain: {error}")
    ...
    if target.stat().st_size != COLOR_BYTES:
        raise ValueError("Mirrorhold RGBA8 color sidecar length mismatch")

    compared = 0
    max_uv_error = 0.0
    for node in published_document.get("nodes", ()):
        name = str(node.get("name", ""))
        if not name.startswith("Terrain_mirrorhold_") or "mesh" not in node:
            continue
        ...
        for position, color, texture_uv in zip(positions, actual, uv):
            ...
            if not (0 <= column < GRID_SHAPE[0] and 0 <= row < GRID_SHAPE[1]):
                raise ValueError(f"Published Mirrorhold vertex is outside saved color grid: {position}")
            if not np.array_equal(rgba8[row, column], np.asarray(color, dtype=np.uint8)):
                raise ValueError(f"Published Mirrorhold color differs at XZ {position[[0, 2]]}")
    ...
    if compared == 0 or max_uv_error > 2.0e-5:
        raise ValueError(
            f"Mirrorhold published terrain validation failed: vertices={compared}, uv={max_uv_error}")
...
def bridge_records(document: dict, body: bytes, roads: dict, output: Path,
    ...
    result, prototypes = [], {}
    for stable_id, names in sorted(PUBLISHED_ASSEMBLIES.items()):
        walk, support = names
        root_names = [walk + "_WorldPlacement", support + "_WorldPlacement"]
        roots = [by_name.get(name, -1) for name in root_names]
        if -1 in roots:
            raise ValueError(f"Published Mirrorhold assembly {stable_id!r} is incomplete")
        ...
        family = f"published-mirror-{slug(stable_id)}"
        ...
        stats = exporter.write()
        asset_id = f"published-mirror-{slug(stable_id)}"
        metadata = {"authoredAccessAssembly": {"id": stable_id,
                                                 "walkNode": walk}}
        if stable_id in sites:
        ...
        record = {
            ...
            "matrix": control.copy(), "collisionRole": "walk_surface",
            "metadata": metadata,
        }
...
def path_records(roads_document: dict, plan: dict, output: Path,
    ...
    if missing := sorted(set(OWNED_ROUTE_IDS) - roads.keys()):
        raise ValueError(f"Published roads are missing Mirrorhold-owned routes: {missing}")
    result = []
    ...
    if len(rivers) != 1 or len(rivers[0].get("points", ())) != 4:
        raise ValueError("Certified Mirrorhold plan must contain the four-point mirror_outlet")
    source = rivers[0]
    ...
    curve = output / "paths/mirror-outlet.tres"
    ...
    river = {
        ...
        "properties": {
            ...
            "name": str(source.get("name", "Mirrorwater")),
...
def marker_records(manifest: dict, objects: list[dict]) -> list[dict]:
    ...
    if len(result) != 129:
        raise ValueError(f"Certified visible gameplay must contain 129 markers, got {len(result)}")
...
def apply_runtime_seed(markers: list[dict], seed: dict) -> None:
    if seed.get("schema") != "eloria-runtime-binding-seed-v1" or \
        ...
        raise ValueError("Runtime binding seed schema/region does not match Mirrorhold")
    ...
    if not seen or len(seen) != int(expected.get("bindings", -1)) or \
        ...
        raise ValueError("Runtime binding seed counts do not match Mirrorhold controls")
...
def write_scene(output: Path, manifest: dict, objects: list[dict], prototypes: dict,
    ...
    lines.extend(("", '[node name="Mirrorhold" type="Node3D"]',
                  'script = ExtResource("region")', f'region_id = "{REGION_ID}"',
                  'continent_translation = Vector3(840, 0, 650)',
                  'metres_per_tile = 1.0', 'server_origin = Vector2i(208, 350)',
                  'server_cells = Vector2i(540, 540)',
                  'collision_origin_metres = Vector2(-208, 350)',
                  ...
                  'owned_plan_feature_ids = PackedStringArray("mirror_lake", "mirror_outlet")',
                  f'runtime_binding_seed_path = {godot_string(runtime_seed_res)}',
                  f'runtime_binding_seed_sha256 = {godot_string(runtime_seed_sha)}',
                  'export_directory = "res://../eloria-assets/maps/nymara-regions/mirrorhold/authoring"',
                  "", '[node name="Terrain" type="Node3D" parent="."]',
                  'script = ExtResource("terrain")', 'origin = Vector2(-208, -190)',
                  'cell_metres = 2.0', 'grid_size = Vector2i(271, 271)',
    ...
    lines.extend(('[node name="Rivers" type="Node3D" parent="."]', "",
                  ...
                  '[node name="WaterRegions" type="Node3D" parent="."]', "",
                  '[node name="MirrorLake" type="Node3D" parent="WaterRegions"]',
                  'script = ExtResource("water")',
                  'position = Vector3(53.6788, 80, 181.4641)',
                  'water_id = "mirror-lake"',
                  'replaces_plan_feature_id = "mirror_lake"',
                  'display_name = "Mirror Lake"',
                  'radii = Vector2(48, 33)',
                  'baseline_plan_depth = 4.0', "",
    ...
    for record in objects:
        ...
        lines.extend((f'[node name={godot_string(node_name(record["id"]))} type="Node3D" parent="AuthoredAssets"]',
                      ...
                      f'catalog_asset_id = {godot_string("mirrorhold:" + record["family"])}',
    ...
    (output / "mirrorhold.tscn").write_text(
...
def main(argv=None) -> int:
    ...
    scene_path = output / "mirrorhold.tscn"
    if scene_path.exists() and not args.force:
        raise ValueError("Refusing to overwrite existing Mirrorhold scene without --force")
    if args.resolved_heights.stat().st_size != GRID_BYTES:
        raise ValueError(f"Mirrorhold resolved grid must contain {GRID_BYTES} bytes")
    if sha256(args.base_heights) != sha256(args.resolved_heights):
        raise ValueError("Mirrorhold uses the certified resolved grid as its explicit saved base")
    ...
    provenance = {
        "schema": "eloria-mirrorhold-authoring-migration-v1", "regionId": REGION_ID,
        "inputs": {
        ...
        "counts": {
            "objects": 168, "publishedAssemblies": len(bridges),
            "savedAssetControls": len(objects), "retiredSourcePlacements": 333,
            "prototypeFamilies": len(prototypes), "paths": len(paths),
            "riverControlPoints": river["sourceControlPointCount"],
            "riverCurvePoints": len(river["points"]), "visibleGameplay": 129,
    ...
    (output / "migration-provenance.json").write_text(
        ...
        encoding="utf-8", newline="\n")
    print(f"mirrorhold_authoring_import_ok scene={scene_path} sha256={provenance['sceneSha256']}")
    return 0
