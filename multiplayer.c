...
void process_message_from_server (const Uint8 *in_data, int data_length)
{
		...
		case LOG_IN_OK:
			{
				...
				safe_snprintf(str, sizeof(str),
					"%c#clientcaps actor16_v1,magic_power_v1,market_window_v1,merchant_window_v1,quest_journal_v1,item_detail_v1,inventory_window_v1,combat_hud_v1,navigation_hud_v1,mail_window_v1,tutorial_window_v1,offline_notifications_v1",
					RAW_TEXT);
				my_tcp_send((Uint8*)str, strlen(str+1)+1);
				break;
			}
	...
	ERR();
		...
		case ELORIA_MAIL_STATE:
			...
			break;
		case ELORIA_TUTORIAL_STATE:
			if (data_length >= 5)
				tutorial_window_update(in_data + 3, data_length - 3);
			else
				LOG_WARNING("Invalid Eloria tutorial packet.\\n");
			break;
