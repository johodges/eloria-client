...
void process_message_from_server (const Uint8 *in_data, int data_length)
{
		...
		case LOG_IN_OK:
			{
				...
				safe_snprintf(str, sizeof(str),
					"%c#clientcaps actor16_v1,magic_power_v1,market_window_v1,merchant_window_v1,quest_journal_v1,item_detail_v1,inventory_window_v1,combat_hud_v1,navigation_hud_v1,mail_window_v1,special_events_v1,offline_notifications_v1",
					RAW_TEXT);
				my_tcp_send((Uint8*)str, strlen(str+1)+1);
				break;
			}
	...
	ERR();
		...
		case ELORIA_MARKETPLACE_STATE:
			if (data_length >= 14)
				marketplace_update(in_data + 3, data_length - 3);
			else
				LOG_WARNING("Invalid Eloria marketplace packet.\n");
			break;
		case ELORIA_MERCHANT_STATE:
			if (data_length >= 19)
		...
		case ELORIA_INVENTORY_STATE:
			...
			break;
		case ELORIA_COMBAT_STATE:
			if (data_length >= 17)
				combat_hud_update(in_data + 3, data_length - 3);
			else
				LOG_WARNING("Invalid Eloria combat HUD packet.\n");
			break;
		case ELORIA_NAVIGATION_STATE:
			if (data_length >= 12)
		...
		case ELORIA_MAIL_STATE:
			...
			break;
		case ELORIA_SPECIAL_EVENT_STATE:
			if (data_length >= 5)
				special_event_window_update(in_data + 3, data_length - 3);
			else
				LOG_WARNING("Invalid Eloria special event packet.\n");
			break;
