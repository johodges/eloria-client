...
bool active=false;std::string minimap_path;float sun[4]={0,0,1,0},sun_light[4]={1,1,1,1},fog_color[4]={0,0,0,1},fog_start=100,fog_end=500;bool fog=false;int collision_w=0,collision_h=0;bool show_collision=false;
...
bool vec2(const json&v,float o[2],const char*n,std::string&e) {
	if(!v.is_array()||v.size()!=2){e=std::string(n)+" must have two numbers";return false;}
	for(int i=0;i<2;i++){if(!v[i].is_number()){e=std::string(n)+" contains a non-number";return false;}o[i]=v[i].get<float>();if(!std::isfinite(o[i])){e=std::string(n)+" contains a non-finite number";return false;}}return true;
}
bool glb(const std::string&p,std::string&e) {
	...
	for(const auto&image:j.value("images",json::array()))if(image.contains("uri")){std::string uri=image["uri"];if(!safe_rel(uri)||!exists(dir(p)+"/"+uri)){e="missing or unsafe external GLB image";return false;}}
...
extern "C" int load_world_package(const char *name,world_update_func *update){
...
#ifndef NDEBUG
	show_collision=std::getenv("ELORIA_WORLD_COLLISION_DEBUG")!=NULL;
#endif
	...
	json env=r.value("environment",json::object());float a[3];if(!vec3(env.value("ambient_color",json::array({.6,.65,.75})),a,"environment.ambient_color",e)){rollback();LOG_ERROR("World package '%s': %s",m.c_str(),e.c_str());return -1;}float sun_source[3];if(!vec3(env.value("sun_direction",json::array({-.4,-.8,-.3})),sun_source,"environment.sun_direction",e)){rollback();return -1;}sun[0]=-sun_source[0];sun[1]=sun_source[2];sun[2]=-sun_source[1];sun[3]=0;float fc[3];if(!vec3(env.value("fog_color",json::array({.7,.75,.8})),fc,"environment.fog_color",e)){rollback();return -1;}for(int k=0;k<3;k++)fog_color[k]=fc[k];fog=env.value("fog_enabled",false);fog_start=env.value("fog_start",100.f);fog_end=env.value("fog_end",500.f);if(!std::isfinite(fog_start)||!std::isfinite(fog_end)||fog_start<0||fog_end<=fog_start){rollback();LOG_ERROR("World package '%s': invalid fog range",m.c_str());return -1;}
	...
	float sc[3];if(!vec3(env.value("sun_color",json::array({1.,1.,1.})),sc,"environment.sun_color",e)){rollback();LOG_ERROR("World package '%s': %s",m.c_str(),e.c_str());return -1;}float si=env.value("sun_intensity",1.0f);if(!std::isfinite(si)||si<0||si>16){rollback();LOG_ERROR("World package '%s': invalid sun intensity",m.c_str());return -1;}for(int k=0;k<3;k++)sun_light[k]=sc[k]*si;sun_light[3]=1;
	json coordinates=r.value("coordinates",json::object());float origin[3];if(!vec3(coordinates.value("origin",json::array({0,0,0})),origin,"coordinates.origin",e)){rollback();LOG_ERROR("World package '%s': %s",m.c_str(),e.c_str());return -1;}float units=coordinates.value("units_per_meter",1.0f);if(!std::isfinite(units)||units<=0||coordinates.value("up_axis","Y")!="Y"||coordinates.value("forward_axis","-Z")!="-Z"){rollback();LOG_ERROR("World package '%s': unsupported coordinates",m.c_str());return -1;}
	json materials=r.value("effects",json::object()).value("materials",json::object());float water_scroll[2],fall_scroll[2];if(!vec2(materials.value("water",json::object()).value("uv_scroll",json::array({0,0})),water_scroll,"effects.materials.water.uv_scroll",e)||!vec2(materials.value("waterfall",json::object()).value("uv_scroll",json::array({0,0})),fall_scroll,"effects.materials.waterfall.uv_scroll",e)){rollback();LOG_ERROR("World package '%s': %s",m.c_str(),e.c_str());return -1;}float pulse=materials.value("landmark-energy",json::object()).value("pulse_hz",0.f);if(!std::isfinite(pulse)||pulse<0||pulse>20){rollback();LOG_ERROR("World package '%s': invalid landmark energy pulse",m.c_str());return -1;}
	if(!world_glb_load(scene_path.c_str(),units,origin)){world_glb_destroy();rollback();LOG_ERROR("World package '%s': GLB render resource creation failed",m.c_str());return -1;}world_glb_set_material_effects(water_scroll[0],water_scroll[1],fall_scroll[0],fall_scroll[1],pulse);
	ambient_r=a[0]*intensity;ambient_g=a[1]*intensity;ambient_b=a[2]*intensity;dungeon=0;std::string mini=r.value("minimap","");if(!mini.empty()){if(!safe_rel(mini)||!exists(dir(m)+"/"+mini)){world_glb_destroy();rollback();LOG_ERROR("World package '%s': invalid or missing minimap",m.c_str());return -1;}minimap_path=dir(m)+"/"+mini;}safe_strncpy(map_file_name,m.c_str(),sizeof(map_file_name));active=true;if(update)update((char*)"Loading portable world",100);LOG_INFO("Loaded package '%s' (%dx%d)",m.c_str(),w,h);return 1;
}
extern "C" void destroy_world_package(){world_glb_destroy();minimap_path.clear();active=false;}extern "C" int world_package_active(){return active?1:0;}
extern "C" void world_package_draw(int transparent){if(!active)return;world_package_apply_environment();world_glb_draw(transparent);if(transparent&&show_collision){glDisable(GL_TEXTURE_2D);glPointSize(3);glColor3f(1,0,0);glBegin(GL_POINTS);for(int y=0;y<collision_h;y++)for(int x=0;x<collision_w;x++)if(height_map[y*collision_w+x]==0)glVertex3f(x*.5f+.25f,y*.5f+.25f,.05f);glEnd();glColor4f(1,1,1,1);}}
extern "C" const char *world_package_minimap(){return minimap_path.c_str();}
extern "C" void world_package_apply_environment(){if(!active)return;float ambient[4]={ambient_r,ambient_g,ambient_b,1};glEnable(GL_LIGHT7);glLightfv(GL_LIGHT7,GL_AMBIENT,ambient);glLightfv(GL_LIGHT7,GL_DIFFUSE,sun_light);set_global_light_position(sun);memcpy(skybox_fog_color,fog_color,sizeof(fog_color));glFogfv(GL_FOG_COLOR,fog_color);glFogf(GL_FOG_START,fog_start);glFogf(GL_FOG_END,fog_end);if(fog)glEnable(GL_FOG);else glDisable(GL_FOG);}
