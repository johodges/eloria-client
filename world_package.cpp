...
extern "C" void world_package_apply_environment(){if(!active)return;set_global_light_position(sun);memcpy(skybox_fog_color,fog_color,sizeof(fog_color));glFogfv(GL_FOG_COLOR,fog_color);glFogf(GL_FOG_START,fog_start);glFogf(GL_FOG_END,fog_end);if(fog)glEnable(GL_FOG);else glDisable(GL_FOG);}
