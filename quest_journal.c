...
static int click_handler(window_info *win,int mx,int my,Uint32 flags){int row=(my-35)/22;(void)win;(void)mx;(void)flags;if(my>=35&&row>=0&&row<entry_count)selected=row;return 1;}
static int close_handler(window_info *win){(void)win;journal_win=-1;return 1;}
void display_quest_journal(void){if(journal_win<0){journal_win=create_window("Quest Journal",game_root_win,0,90,55,800,510,ELW_USE_UISCALE|ELW_WIN_DEFAULT);set_window_handler(journal_win,ELW_HANDLER_DISPLAY,&display_handler);set_window_handler(journal_win,ELW_HANDLER_CLICK,&click_handler);set_window_handler(journal_win,ELW_HANDLER_CLOSE,&close_handler);}else show_window(journal_win);}
